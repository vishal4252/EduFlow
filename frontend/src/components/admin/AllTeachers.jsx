...
export default function AllTeachers() {
  ...
  const filteredTeachers = useMemo(() => {
    ...
    return teachers.filter(
      (teacher) =>
        teacher.username?.toLowerCase().includes(value) ||
  ...
  const handleDeleteTeacher = async () => {
    ...
    try {
      setDeletingId(deletingTeacher._id);

      const data = await deleteUser(deletingTeacher._id);

      setTeachers((prevTeachers) =>
        prevTeachers.filter((teacher) => teacher._id !== deletingTeacher._id),
      );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              >
                {deletingId !== null ? "Deleting..." : "Delete Teacher"}
              </button>
            </div>
          </div>
        </div>
